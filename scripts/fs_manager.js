...
class FileSystemManager {
    ...
    /**
     * Saves the entire filesystem by fetching the state from the Python kernel
     * and persisting it to storage.
    ...
    async save() {
        ...
        // Fallback to old method if kernel isn't ready (shouldn't happen in normal operation)
        console.warn("FSManager.save: Kernel not ready, using JS-only save as fallback.");
        const saveData = this.dependencies.Utils.deepCopyNode(this.fsData);
        const success = await this.storageHAL.save(saveData);
        if (success) {
            return ErrorHandler.createSuccess();
        }
    ...
    /**
     * The load method is now primarily handled during boot in main.js.
     * This function remains to ensure the JS manager has a default state if needed,
     * but the authoritative state is loaded into Python.
     * @returns {Promise<object>} A promise that resolves to a success or error object.
     */
    async load() {
        const { ErrorHandler, OutputManager } = this.dependencies;
        // The actual loading from StorageHAL and hydrating the Python kernel
        // now happens in main.js. This method's role is reduced.
        // We will just ensure a default FS exists on the JS side.
        if (Object.keys(this.fsData).length === 0) {
            await this.initialize(this.config.USER.DEFAULT_NAME);
            await this.save(); // Save the initial JS state
        }
        return ErrorHandler.createSuccess();
    }
    ...
    // ... (the rest of fs_manager.js remains the same) ...
