...
def _format_long(path, name, node):
    ...
    if node.get('type') == 'directory':
        type_char = "d"
    elif node.get('type') == 'symlink':
        type_char = "l"

    # --- FULL PERMISSION PARSING ---
    # Owner permissions
    owner_perms = 'r' if (mode >> 8) & 1 else '-'
    owner_perms += 'w' if (mode >> 7) & 1 else '-'
    owner_perms += 'x' if (mode >> 6) & 1 else '-'

    # Group permissions
    group_perms = 'r' if (mode >> 5) & 1 else '-'
    group_perms += 'w' if (mode >> 4) & 1 else '-'
    group_perms += 'x' if (mode >> 3) & 1 else '-'

    # Other permissions
    other_perms = 'r' if (mode >> 2) & 1 else '-'
    other_perms += 'w' if (mode >> 1) & 1 else '-'
    other_perms += 'x' if (mode >> 0) & 1 else '-'
    ...
    # Handle symlinks in long format
    display_name = name
    if node.get('type') == 'symlink':
        display_name += f" -> {node.get('target', '')}"
...
def run(args, flags, user_context, stdin_data=None):
    ...
    for i, path in enumerate(paths):
        ...
        # For symlinks, we want to show info about the link itself with -l, not the target
