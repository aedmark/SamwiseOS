...
def _format_long(path, name, node):
    ...
    perms += "rwx" * 3  # Simplified representation
    ...
    for i, path in enumerate(paths):
        ...
        if node.get('type') == 'directory':
            if len(paths) > 1 and i > 0:
                output.append("") # Add a newline between directory listings
            if len(paths) > 1:
...
def help(args, flags, user_context, stdin_data=None):
    ...
    """
    return "Usage: ls [-a] [-l] [-R] [-t] [-S] [-X] [-r] [FILE...]"